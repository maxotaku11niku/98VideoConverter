...
void VideoConverterEngine::EncodeVideo(char* outFileName, bool (*progressCallback)(unsigned int))
{
	ResetPlaneData();
	...
	writefile = fopen(outFileName, "wb");
...
void VideoConverterEngine::OpenForDecodeVideo(char* inFileName)
{
	//Open video file
	if (alreadyOpen) CloseDecoder();
	avformat_open_input(&fmtcontext, inFileName, NULL, NULL);
